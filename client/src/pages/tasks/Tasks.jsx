...
function Tasks({
  ...
  const handleSort = (param) => {
    if (param === "deadline-nearest")
      tasks.sort((a, b) =>
        moment(a.deadline).isAfter(moment(b.deadline)) ? 1 : -1
      );
    else if (param === "deadline-farthest")
      tasks.sort((a, b) =>
        moment(a.deadline).isBefore(moment(b.deadline)) ? 1 : -1
      );
            ...
            <tbody>
              {tasks.map((singletask) => {
                ...
                return (
                  <tr
                    key={uuid}
                    onClick={() => handleRowClick(project_uuid, project_name)}
                  >
                    <td className="task-name">{task}</td>
                    <td
                      className={`deadline ${
                        moment(deadline).isBefore() ? "red" : ""
                      }`}
                    >
